...
class latentGenerator(Generator):
    def __init__(self, h, xv_target=None, f0_target=None):
        super().__init__(h)
        self.xv_target = xv_target
    ...
    @torch.no_grad()
    def f0_replace(self, f0_source, f0_target):
        f0_source_numpy = f0_source.cpu().numpy()
        for i in range(f0_source_numpy.shape[0]):
            f0_new = f0_source_numpy[i][0]
            mask = ~np.isclose(f0_new, 0.0) # discard 0 values
            values = f0_new[mask]
            # if a.f0_log:
            ...
            if not np.isclose(values_std, 0.0):
                values *= std / values_std
            # endif
            values += mean
            # if a.f0_log:
            values = np.exp(values)
            # endif
            f0_new = torch.tensor(values, device=f0_source.device)
            f0_source[i,0,mask] = f0_new
        return f0_source

    def forward(self, **kwargs):
        audio_data = kwargs['audio']
        print(f'debug self.latent {bool(self.latent)}')
        print(f'debug self.xv {bool(self.xv)}')
        if self.latent and self.xv:
            ...
            with torch.no_grad():
                if self.xv_feature == 'fbank':
                    # fbank input (batchsize,wav_len)
                        ...
                        xv, _ = self.xv_model(xv_input) # originally outside torch.no_grad()
            ...
            if self.xv_target is not None:
                print('substituting xvector\n', end='')
        ...
        if self.f0:
            if self.f0_target is None:
                f0 = kwargs['f0']
            else:
                print('substituting f0\n', end='')
                with torch.no_grad():
                    f0 = self.f0_replace(kwargs['f0'], self.f0_target) # TODO: check
