...
def init_worker(arguments, file_list):
    import logging
...
@torch.no_grad()
def f0_replace(f0, target): # NOTE: integrity of f0 is not preserved
    """
    replace F0
    inputs:
    ...
    if a.no_f0:
        return f0
    target_f0_name = os.path.join(str(a.f0_path), f'{target}.npy')
...
@torch.no_grad()
def inference(item_index, trials_df):
    ...
    try:
        val = trials_df.loc[fname_out_name].values # retrieve entries matching with the utterance
    except KeyError as ke:
        # audio not to convert, ignored
        return
    if len(val.shape) == 1: # shape check
        val = val.reshape((1,-1))
    for target, is_target in val: # for each target of the given source (utterance)
        # target: id of target, is_target: are source and target the same speaker
        if is_target == 'nontarget': # convert
            x['f0'] = f0_replace(x['f0'], target)
            gt_audio = torch.autograd.Variable(gt_audio.to(device, non_blocking=False))
            xv_path = os.path.join(str(a.xv_path), f'{target}.xvector')
            audio, rtf = generate(h, generator, x, xv_path)
            audio = librosa.util.normalize(audio.astype(np.float32))
        else: # do not convert
            audio = gt_audio.cpu().numpy().astype(np.float32) # do not modify audio in this case
        output_file = os.path.join(a.output_dir, f'{fname_out_name}_{target}.wav') # save as sourceFilename_targetId.wav
...
def main():
    ...
    parser.add_argument('--input_trials_file', required=True, default=None, help='trials file, e.g data/libri_dev/trials_f')
    ...
    parser.add_argument('--output_dir', default='generated_files', help='directory where to save generated audios')
    parser.add_argument('--checkpoint_file', required=True, help='gan model parameters')
    parser.add_argument('--xv_path', type=Path, default=None, required=True, help='directory with target speaker vectors (as saved by xv_extract/ or xvector_mean.py)')
    parser.add_argument('--f0_path', type=Path, default=None, required=False, help='directory with target F0s (as saved by f0_extract.py or f0_avg.py)')
    ...
    # read trials_file and convert to dataframe grouped by utterance
    trials_list = []
    for line in open(a.input_trials_file):
        trials_list.append(line.strip().split())
    trials_df = pd.DataFrame(trials_list, columns=['target', 'utt', 'is_target'])
    trials_df.set_index('utt', inplace=True)
    
    init_worker(a, file_list)

    for i in range(0, len(dataset)):
        inference(i, trials_df)
