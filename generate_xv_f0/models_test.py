...
class latentGenerator(Generator):
    def __init__(self, h, xv_path=None):
        self.xv_path = xv_path
        super().__init__(h)
    ...
    def gen_vpc(self, xv_path,**kwargs):
        ...
        if self.latent and self.xv:
            ...
            # this if statement should never be executed
            if self.xv_path is None:
                # extract xvector
                if self.xv_feature == 'fbank':
                    # fbank input (batchsize,wav_len)
                    with torch.no_grad():
                ...
                # read target xvector and use it for the generation
